...
class TaskManager:
    ...
    def __update_process(self, pid:int, log_file, device):
        ...
        if not constants.IS_WIN:
            args = shlex.split(args)
        if self._idx % 200 == 0:
            Popen(["C:\\Users\\temp\\vpudriver-31.0.100.1351-RelWithDebInfo\\RelWithDebInfo\\scripts\\vpudriverpnp.bat", "disable_device"], shell=constants.IS_WIN)
            time.sleep(5)
            Popen(["C:\\Users\\temp\\vpudriver-31.0.100.1351-RelWithDebInfo\\RelWithDebInfo\\scripts\\vpudriverpnp.bat", "enable_device"], shell=constants.IS_WIN)
