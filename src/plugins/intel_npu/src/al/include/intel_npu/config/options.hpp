...
struct NPU_VM_RUNTIME_MODE final : OptionBase<NPU_VM_RUNTIME_MODE, ov::intel_npu::VmRuntimeMode> {
    static std::string_view key() {
        return ov::intel_npu::vm_runtime_mode.name();
    }

    static constexpr std::string_view getTypeName() {
        return "ov::intel_npu::VmRuntimeMode";
    }

    static ov::intel_npu::VmRuntimeMode defaultValue() {
        return ov::intel_npu::VmRuntimeMode::JIT;
    }

    static ov::intel_npu::VmRuntimeMode parse(std::string_view val) {
        if (val == "JIT") {
            return ov::intel_npu::VmRuntimeMode::JIT;
        } else if (val == "INTERPRETER") {
            return ov::intel_npu::VmRuntimeMode::INTERPRETER;
        }

        OPENVINO_THROW("Value '", val, "' is not a valid NPU_VM_RUNTIME_MODE option");
    }

    static std::string toString(const ov::intel_npu::VmRuntimeMode& val) {
        switch (val) {
        case ov::intel_npu::VmRuntimeMode::JIT:
            return "JIT";
        case ov::intel_npu::VmRuntimeMode::INTERPRETER:
            return "INTERPRETER";
        default:
            OPENVINO_THROW("No valid string for current NPU_VM_RUNTIME_MODE option");
        }
    }
